...
function screenToWorld(e) {
  ...
  const scale = getScale();
  return { x: (x - VIEW.offsetX) / scale, y: (y - VIEW.offsetY) / scale };
}
...
function getTouchCenter(touches) {
  ...
  for (let i = 0; i < touches.length; i++) {
    ...
    sumX += touch.clientX; sumY += touch.clientY;
  }
...
function draw() {
  ...
  for (const shape of objects.shapes ?? []) { drawShape(ctx, shape); }
  for (const line of objects.lines ?? []) {
    if (line.kind === "free") { drawPolyline(ctx, line.points || [], line.color, line.width ?? DEFAULT_STROKE_WIDTH, line.dash ?? []); }
    else { drawArrowLine(ctx, line.x1, line.y1, line.x2, line.y2, line.color, line.arrow, line.width ?? DEFAULT_STROKE_WIDTH, line.dash ?? []); }
  }
  if (previewShape) {
  ...
  if (previewLine) {
    ctx.save(); ctx.globalAlpha = 0.85;
    drawArrowLine(ctx, previewLine.x1, previewLine.y1, previewLine.x2, previewLine.y2, previewLine.color, previewLine.arrow, previewLine.width, previewLine.dash);
    ctx.restore();
  }
  ...
  for (const tower of objects.towers ?? []) { drawTower(ctx, tower.x, tower.y, tower.sprite); }
  for (const flag of objects.flags ?? []) { drawFlag(ctx, flag.x, flag.y, flag.sprite); }
  for (const marker of objects.markers ?? []) { drawMarker(ctx, marker.x, marker.y, marker.color, marker.text); }
  for (const note of objects.texts ?? []) { drawTextNote(ctx, note); }
  endWorld();
  updateZoomIndicator();
}
...
function updateCursor() {
  ...
  else if (isPanKey) { canvas.style.cursor = "grab"; }
  else if (mode === "text") { canvas.style.cursor = "text"; }
  else if (mode === "eraser") { canvas.style.cursor = "not-allowed"; }
  else if (mode === "drawLine" || mode === "freehand" || mode.startsWith("shape")) { canvas.style.cursor = "crosshair"; }
  else { canvas.style.cursor = "default"; }
}
function setMode(newMode) {
  const target = mode === newMode ? "idle" : newMode;
  mode = target;
  MODE_BUTTONS.forEach((button, key) => { if (!button) return; button.classList.toggle("active", mode === key); });
  if (mode !== "drawLine") { lineStart = null; previewLine = null; }
  if (mode !== "freehand") { isDrawingFree = false; freePoints = []; }
  if (!mode.startsWith("shape")) { shapeStart = null; previewShape = null; }
  if (mode !== "eraser") { isErasing = false; eraserChanged = false; }
  updateCursor();
}
...
function updatePanFromPointer(pointer) {
  ...
  VIEW.offsetX += current.x - panLast.x; VIEW.offsetY += current.y - panLast.y;
  panLast = current; clampView(); draw();
}
...
function scaleObjects(obj, fw, fh, tw, th) {
  ...
  const offsetX = (tw - fw * scale) / 2; const offsetY = (th - fh * scale) / 2;
  const applyPoint = (point) => { if (!point) return; point.x = point.x * scale + offsetX; point.y = point.y * scale + offsetY; };
  for (const tower of obj.towers ?? []) { applyPoint(tower); }
  for (const flag of obj.flags ?? []) { applyPoint(flag); }
  for (const marker of obj.markers ?? []) { applyPoint(marker); }
  for (const line of obj.lines ?? []) {
    if (line.kind === "free") { for (const point of line.points ?? []) { applyPoint(point); } }
    else { line.x1 = line.x1 * scale + offsetX; line.y1 = line.y1 * scale + offsetY; line.x2 = line.x2 * scale + offsetX; line.y2 = line.y2 * scale + offsetY; }
    line.width = (line.width ?? DEFAULT_STROKE_WIDTH) * scale;
  }
  for (const shape of obj.shapes ?? []) {
    applyPoint(shape);
    shape.width = (shape.width ?? 0) * scale; shape.height = (shape.height ?? 0) * scale;
    shape.strokeWidth = (shape.strokeWidth ?? DEFAULT_STROKE_WIDTH) * scale;
  }
  for (const text of obj.texts ?? []) { applyPoint(text); text.fontSize = (text.fontSize ?? 18) * scale; }
}
function swapSpritesByRegion() {
  const mid = WORLD.w / 2;
  for (const t of objects.towers ?? []) {
    const left = t.x < mid; const color = (left ^ teamSwap) ? "blue" : "red";
    t.sprite = color === "blue" ? "tower_blue" : "tower_red";
  }
  for (const f of objects.flags ?? []) {
    const left = f.x < mid; const color = (left ^ teamSwap) ? "blue" : "red";
...
function fitCanvas(redraw = true) {
  ...
  const width = Math.max(240, rect.width); const height = Math.max(200, rect.height);
  const center = getViewCenterWorld();
  canvas.width = Math.round(width); canvas.height = Math.round(height);
  VIEW.baseScale = Math.min(canvas.width / WORLD.w, canvas.height / WORLD.h) || 1;
  if (center) { centerViewOn(center.x, center.y); }
  else { centerViewOn(WORLD.w / 2, WORLD.h / 2); }
  if (redraw) draw();
}
function pickList(hit) {
  if (!hit) return null;
  if (hit.type === "tower") return objects.towers; if (hit.type === "flag") return objects.flags;
  if (hit.type === "marker") return objects.markers; if (hit.type === "shape") return objects.shapes;
  if (hit.type === "text") return objects.texts;
  return null;
}
function addTower(sprite) {
  const center = getViewCenterWorld();
  const position = center ?? { x: WORLD.w * 0.2 + Math.random() * 40, y: WORLD.h * 0.3 + Math.random() * 40, };
  objects.towers.push({ x: position.x, y: position.y, sprite }); draw(); commitChange();
}
function addFlag(sprite) {
  const center = getViewCenterWorld();
  const position = center ?? { x: WORLD.w * 0.2 + Math.random() * 40, y: WORLD.h * 0.4 + Math.random() * 40, };
  objects.flags.push({ x: position.x, y: position.y, sprite }); draw(); commitChange();
}
function addMarker() {
  const center = getViewCenterWorld(); const position = center ?? { x: WORLD.w / 2, y: WORLD.h / 2 };
  objects.markers.push({ x: position.x, y: position.y, color: selectedMarkerColor, text: "1", });
  draw(); commitChange();
}
function createTextAt(x, y, existing) {
  const defaultValue = existing?.text ?? ""; const value = prompt("輸入文字內容：", defaultValue);
  if (value === null) return false; const text = value.trim();
  if (!text) { if (existing) { existing.text = ""; draw(); commitChange(); } return false; }
  if (existing) { existing.text = text; draw(); commitChange(); return true; }
  objects.texts.push({ x, y, text, color: selectedTextColor, fontSize: getTextSize(), align: "left", });
  draw(); commitChange(); return true;
}
function promptMarkerText(marker) {
  const value = prompt("輸入標記內容：", marker.text ?? ""); if (value === null) return false;
  marker.text = value; draw(); commitChange(); return true;
}
function undo() {
  if (history.length <= 1) return; const current = history.pop();
  redoStack.push(current); const previous = history[history.length - 1];
  loadFromSnapshot(previous);
}
function redo() {
  if (!redoStack.length) return; const next = redoStack.pop();
  history.push(next); loadFromSnapshot(next);
}
function handlePanKey(e) {
  if (e.code === "Space") {
    if (e.type === "keydown") { if (!isPanKey) { isPanKey = true; updateCursor(); } e.preventDefault(); }
    else if (e.type === "keyup") { isPanKey = false; stopPan(); updateCursor(); }
  }
}
function onWheel(e) {
  e.preventDefault(); const delta = e.deltaY;
  ...
  const prevZoom = VIEW.zoom; if (Math.abs(newZoom - prevZoom) < 0.001) return;
  const canvasPoint = getCanvasPoint(e); const scale = getScale();
...
function onCanvasPointerDown(e) {
  ...
  if (button !== 0) return; const point = screenToWorld(e); isDragging = false;
  if (mode.startsWith("shape")) {
    const type = mode === "shape:circle" ? "circle" : "rect"; shapeStart = { x: point.x, y: point.y, type };
    previewShape = { type, x: point.x, y: point.y, width: 0, height: 0, color: selectedLineColor, strokeWidth: selectedLineWidth, dash: selectedLineDash.slice() };
  } else if (mode === "text") { createTextAt(point.x, point.y); }
  else if (mode === "drawLine") { if (!lineStart) { lineStart = { x: point.x, y: point.y }; previewLine = null; } }
  else if (mode === "freehand") { isDrawingFree = true; freePoints = [{ x: point.x, y: point.y }]; draw(); }
  else if (mode === "eraser") {
    isErasing = true; eraserChanged = false; const idx = nearestLineIndex(objects.lines, point.x, point.y, ERASER_HIT_RADIUS);
    if (idx >= 0) { objects.lines.splice(idx, 1); eraserChanged = true; draw(); }
  } else {
    const hit = hitTest(ctx, objects, point.x, point.y);
    if (hit) {
      const list = pickList(hit); if (!list) return; dragTarget = { type: hit.type, idx: hit.idx };
      const obj = list[hit.idx]; dragOffset.x = point.x - (obj.x ?? point.x); dragOffset.y = point.y - (obj.y ?? point.y);
      mode = "drag"; deleteZone?.classList.add("showing");
    } else { startPan(e); mode = "idle"; }
  }
  updateCursor(); window.addEventListener("mousemove", onWindowPointerMove); window.addEventListener("mouseup", onWindowPointerUp);
}
function onWindowPointerMove(e) {
  if (isPanning) { updatePanFromPointer(e); return; }
  if (mode === 'idle') return; const point = screenToWorld(e);
  if (mode === "eraser" && isErasing) {
    const idx = nearestLineIndex(objects.lines, point.x, point.y, ERASER_HIT_RADIUS);
    if (idx >= 0) { objects.lines.splice(idx, 1); eraserChanged = true; draw(); } return;
  }
  if (shapeStart && previewShape) {
    const left = Math.min(shapeStart.x, point.x); const top = Math.min(shapeStart.y, point.y);
    const width = Math.abs(point.x - shapeStart.x); const height = Math.abs(point.y - shapeStart.y);
    previewShape = { type: shapeStart.type, x: left + width / 2, y: top + height / 2, width, height, color: selectedLineColor, strokeWidth: selectedLineWidth, dash: selectedLineDash.slice() };
    draw(); return;
  }
  if (mode === "drawLine" && lineStart) {
    previewLine = { x1: lineStart.x, y1: lineStart.y, x2: point.x, y2: point.y, color: selectedLineColor, arrow: arrowType?.value ?? "none", width: selectedLineWidth, dash: selectedLineDash.slice() };
    draw(); return;
  }
  if (mode === "freehand" && isDrawingFree) { freePoints.push({ x: point.x, y: point.y }); draw(); return; }
  if (mode === "drag" && dragTarget) {
    const list = pickList(dragTarget); if (!list) return; const obj = list[dragTarget.idx];
    obj.x = point.x - dragOffset.x; obj.y = point.y - dragOffset.y; isDragging = true;
    const isOver = isPointerOverDeleteZone(e); deleteZone?.classList.toggle("active", isOver);
    draw();
  }
}
function onWindowPointerUp(e) {
  const isOverDeleteZone = isPointerOverDeleteZone(e);
  deleteZone?.classList.remove("showing"); deleteZone?.classList.remove("active");
  if (isPanning) { stopPan(); window.removeEventListener("mousemove", onWindowPointerMove); window.removeEventListener("mouseup", onWindowPointerUp); return; }
  const point = screenToWorld(e);
  if (shapeStart && previewShape) {
    if (previewShape.width > MIN_SHAPE_SIZE && previewShape.height > MIN_SHAPE_SIZE) {
      objects.shapes.push({ type: previewShape.type, x: previewShape.x, y: previewShape.y, width: previewShape.width, height: previewShape.height, color: previewShape.color, strokeWidth: previewShape.strokeWidth, dash: previewShape.dash.slice() });
      draw(); commitChange();
    }
    shapeStart = null; previewShape = null;
  } else if (mode === "drawLine" && lineStart) {
    objects.lines.push({ x1: lineStart.x, y1: lineStart.y, x2: point.x, y2: point.y, color: selectedLineColor, arrow: arrowType?.value ?? "none", width: selectedLineWidth, dash: selectedLineDash.slice() });
    lineStart = null; previewLine = null; draw(); commitChange();
  } else if (mode === "freehand" && isDrawingFree) {
    if (freePoints.length > 1) {
      objects.lines.push({ kind: "free", points: freePoints.slice(), color: selectedLineColor, width: selectedLineWidth, dash: selectedLineDash.slice() });
      draw(); commitChange();
    }
    isDrawingFree = false; freePoints = [];
  } else if (mode === "eraser" && isErasing) {
    if (eraserChanged) { commitChange(); }
    isErasing = false; eraserChanged = false;
  } else if (mode === "drag" && dragTarget) {
    const list = pickList(dragTarget);
    if (list) {
      if (isOverDeleteZone) { list.splice(dragTarget.idx, 1); draw(); commitChange(); }
      else if (!isDragging) {
        const obj = list[dragTarget.idx];
        if (dragTarget.type === "marker") { promptMarkerText(obj); }
        else if (dragTarget.type === "text") { createTextAt(obj.x, obj.y, obj); }
      } else { commitChange(); }
    }
    mode = "idle"; dragTarget = null; isDragging = false; updateCursor();
  } else if (mode === "idle") {
    const hit = hitTest(ctx, objects, point.x, point.y);
    if (hit && hit.type === "text" && !isDragging) { const list = pickList(hit); createTextAt(point.x, point.y, list?.[hit.idx]); }
  }
  window.removeEventListener("mousemove", onWindowPointerMove); window.removeEventListener("mouseup", onWindowPointerUp);
}
...
async function loadSlots() {
  try {
    const stored = await localforage.getItem(SLOT_KEY);
...
async function persistSlots() {
  try {
    ...
    await localforage.setItem(SLOT_KEY, toSave);
  } catch (err) {
    console.error("儲存插槽失敗:", err);
    alert("儲存失敗！瀏覽器儲存空間可能已滿或發生錯誤。");
  }
}
...
function renderSlots() {
  ...
  for (const slot of slots) {
    ...
    option.value = slot.id; option.textContent = slot.data ? `${slot.name} ✓` : slot.name;
...
async function loadMaps() {
  try {
    ...
    if (Array.isArray(stored)) { maps = [DEFAULT_MAP, ...stored.filter((m) => m && m.src)]; }
  } catch (err) { console.warn("讀取地圖列表失敗：", err); maps = [DEFAULT_MAP]; }
  const savedId = await localforage.getItem(MAP_CURRENT_KEY_LF);
  if (savedId && maps.some((m) => m.id === savedId)) { currentMapId = savedId; }
  else { currentMapId = DEFAULT_MAP.id; }
}
async function saveMaps() {
  const userMaps = maps.filter((m) => !m.builtIn);
  try { await localforage.setItem(MAPS_KEY_LF, userMaps); }
  catch (err) { console.error("儲存地圖失敗 (可能空間不足):", err); alert("儲存地圖失敗！瀏覽器儲存空間可能已滿。"); }
}
function generateMapId() {
  if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") { return `map-${crypto.randomUUID()}`; }
  let random = ""; for (let i = 0; i < 4; i++) { random += Math.random().toString(36).slice(2, 6); }
  return `map-${Date.now().toString(36)}-${random}`;
}
function renderMaps() {
  if (!mapSelect) return; mapSelect.innerHTML = "";
  for (const map of maps) {
    const option = document.createElement("option"); option.value = map.id; option.textContent = map.name;
    mapSelect.appendChild(option);
  }
  mapSelect.value = currentMapId; updateMapButtons();
}
...
async function setCurrentMap(id) {
  ...
  currentMapId = map.id; if (mapSelect) mapSelect.value = currentMapId;
  updateMapButtons();
  if (BG.src !== map.src) { BG.src = map.src; }
  else if (BG.complete) { BG.dispatchEvent(new Event("load")); }
